  ...
  class InvoiceService
    ...
    def self.build_new_invoice(params)
      ...
      if params[:invoice_for_client]
        ...
        invoice = Billing::Invoice.new({:invoice_number => ::Invoice.get_next_invoice_number(nil), :invoice_date => Date.today.strftime(date_format), :client_id => params[:invoice_for_client], :payment_terms_id => (PaymentTerm.all.present? && PaymentTerm.first.id), :company_id => company_id})
        3.times { invoice.invoice_line_items.build() }
      elsif params[:id]
        invoice = Billing::Invoice.find(params[:id]).use_as_template
        invoice.invoice_line_items.build()
      else
        invoice = Billing::Invoice.new({:invoice_number => Billing::Invoice.get_next_invoice_number(nil), :invoice_date => Date.today.strftime(date_format)})
        3.times { invoice.invoice_line_items.build() }
      end
      invoice
    end
    ...
    def self.build_new_project_invoice(project)
      date_format = self.new.date_format
      project.invoices.new({:invoice_number => Billing::Invoice.get_next_invoice_number(nil), :invoice_date => Date.today.strftime(date_format), :payment_terms_id => (PaymentTerm.all.present? && PaymentTerm.first.id), client: project.client, currency: project.client.currency, invoice_type: "ProjectInvoice"})
    end
    ...
    def self.get_company_id(client_id)
      entities = Billing::Client.select('company_entities.parent_id').
