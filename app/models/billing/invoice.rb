...
#  due_date            :date
#  last_invoice_status :string(255)
...
module Billing
  class Invoice < ActiveRecord::Base
    ...
    has_many :invoice_line_items, :dependent => :destroy, class_name: 'Billing::InvoiceLineItem', foreign_key: :invoice_id
    has_many :payments
    has_many :sent_emails, :as => :notification
    has_many :credit_payments, :dependent => :destroy
    has_many :invoice_tasks, :dependent => :destroy

    accepts_nested_attributes_for :invoice_line_items, :reject_if => proc { |line_item| line_item['item_name'].blank? }, :allow_destroy => true
    ...
    def set_default_currency
      self.currency = self.department.setting.currency || 'USD'
    end
    ...
    def paid_amount
      payments.sum(:payment_amount) || 0
    end

    def due_amount
      (self.invoice_total || 0) - self.paid_amount
    end

    def update_invoice_status(payment)
      if (self.invoice_total % self.paid_amount) == 0

        self.update_attributes(status: 'paid')
        payment.update_attributes(paid_full: true)
      else
        self.update_attributes(status: 'partial')
      end
    ...
    # def notify(current_employee, id = nil)
    #   InvoiceMailer.delay.new_invoice_email(self.client, self, self.encrypted_id, current_employee)
    # end
