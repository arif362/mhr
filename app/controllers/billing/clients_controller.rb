...
module Billing
  class ClientsController < ApplicationController
    ...
    def index
      ...
      @clients = current_department.clients.order(created_at: :DESC)
    ...
    # POST /clients.json
    def create
      @client = current_department.clients.build(client_params)
      # company_id = current_department.id
      ...
      respond_to do |format|
        if @client.save
          ...
          #redirect_to({:action => "index", :controller => "clients", :id => @client.id}, :notice => new_client_message) unless params[:quick_create]
          redirect_to billing_clients_path, notice: 'Client has been created successfully.'
          return
        else
    ...
    # PUT /clients/1.json
    def update
      @client = Billing::Client.find_by_id(params[:id])
    ...
    def destroy
      ...
      respond_to do |format|
        format.html { redirect_to billing_clients_path, notice: 'Client was successfully deleted.' }
        format.json { head :no_content }
      end
    end
    ...
    def get_args(status)
      {status: status, per: @per_page, user: current_employee, sort_column: sort_column, sort_direction: sort_direction, current_company: session['current_company'], company_id: current_department.id}
    end

    private

    def client_params
      params.require(:billing_client).permit(:address_street1, :address_street2, :business_phone, :city,
